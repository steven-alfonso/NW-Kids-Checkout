...
describe('admin-guest-entries', () => {
    describe('tabs', () => {
        it('loads only the needs-entry tab on initial page load', async () => {
            ...
            expect(urls.filter(u => u.includes('pending')).length).toBe(1);
        });
        ...
        it('shows an empty state when a tab has no entries', async () => {
            const window = loadWindow();
            await window.loadEntries('entered');
            const view = window.document.getElementById('view-entered');
        ...
        it('renders entries into the active tab view', async () => {
            const window = loadWindow({
                respond: (url) => {
                    if (url.includes('pending') || url.includes('pending')) return { ok: true, status: 200, json: async () => pageOf([entry('p-1', 'pending'), entry('p-2', 'pending')]) };
                    if (url.includes('status=pending')) return { ok: true, status: 200, json: async () => pageOf([entry('p-1', 'pending')]) };
                    return { ok: true, status: 200, json: async () => pageOf([entry('a-1', 'pending')]) };
                }
            });
            await window.loadEntries('needs-entry');
            const view = window.document.getElementById('view-needs-entry');
            expect(view.querySelectorAll('.rounded-xl').length).toBeGreaterThanOrEqual(1);
        });
    ...
    describe('entry rendering', () => {
        ...
        it('renders address summary and safety ack badge', () => {
            const window = loadWindow();
            const container = window.document.createElement('div');
            const sub = entry('sub-ack', 'pending', {
                parent: { first_name: 'A', last_name: 'B', phone: '555', email: 'a@b.com', address1: '123 Main St', address2: '', city: 'Seattle', state: 'WA', zip: '98101' },
                safety_ack: true,
                children: []
            });
            ...
            const sub2 = entry('sub-noack', 'pending', { safety_ack: false });
        ...
        it('renders missing phone/email as an em dash on parent chips', () => {
            const window = loadWindow();
            const container = window.document.createElement('div');
            const sub = entry('sub-4', 'pending', { parent: { first_name: 'A', last_name: 'B', phone: '', email: '' } });
            window.renderEntry(container, sub);
        ...
        it('shows a Copied tooltip on the field after copying', () => {
            const window = loadWindow();
            const container = window.document.createElement('div');
            window.renderEntry(container, entry('sub-3', 'pending'));
        ...
        it('uses distinct badge colors per bucket', () => {
            const window = loadWindow();
            const container = window.document.createElement('div');
            window.renderEntry(container, entry('sub-2', 'pending'));
        ...
        it('adds a status-colored left bar to needs-entry cards', () => {
            const window = loadWindow();
            const container = window.document.createElement('div');
            window.renderEntry(container, entry('pend-1', 'pending'));
            window.renderEntry(container, entry('pend-2', 'pending', { parent: { first_name: 'E', last_name: 'F', phone: '3', email: 'e@f.com' } }));
    ...
    describe('pagination', () => {
        ...
        it('renders page summary and navigation in both top and bottom bars', async () => {
            const window = loadWindow({
                respond: (url) => {
                    if (url.includes('pending') || url.includes('pending')) return { ok: true, status: 200, json: async () => pageOf([entry('p-1', 'pending'), entry('p-2', 'pending')], { total: 15, total_pages: 2 }) };
            ...
            const summary = 'Page 1 of 2 · 15 entries';
        ...
        it('hides pagination bars when a tab has no entries', async () => {
            const window = loadWindow();
            await window.loadEntries('entered');
            expect(window.document.getElementById('pagination-entered-top').classList.contains('hidden')).toBe(true);
            expect(window.document.getElementById('pagination-entered-bottom').classList.contains('hidden')).toBe(true);
        });
        ...
        it('sorts pending entries newest first', async () => {
            const window = loadWindow({
                respond: (url) => {
                    if (url.includes('pending') || url.includes('pending')) {
                        return {
                            ok: true, status: 200, json: async () => pageOf([
                                ...
                                entry('new-pending', 'pending', {
                                    created_at: '2026-02-01T10:00:00Z',
                                    parent: { first_name: 'New', last_name: 'Pending', phone: '1', email: 'a@b.com' }
                                }),
                                entry('no-date-pending', 'pending', {
                                    created_at: null,
        ...
        it('redirects to login when loadEntries returns HTML content-type', async () => {
            const window = loadWindow({
                respond: () => ({ ok: false, redirected: false, url: 'http://localhost/v1/admin/guest-submissions?status=pending&page=1', status: 200, headers: { get: () => 'text/html; charset=utf-8' }, json: async () => { throw new Error('should not parse'); } })
            });
            await window.loadEntries('entered');
            await flush();
            const statusEl = window.document.getElementById('page-status');
