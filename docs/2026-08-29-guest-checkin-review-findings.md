...
- [ ] **16. [low] DOB future-check mixes zones (UTC-parsed date vs local `time.Now()`)**
...
- [ ] **17. [low] No server-side length caps on text fields**
...
- [ ] **18. [low] FK violation detected by brittle string match, dropping the error chain**
...
- [ ] **19. [low] Untested error branches (4 items)**
  - Where/fix — add tests:
...
- [ ] **20. [low] Concurrent approve can surface 500 instead of the conflict path (`SQLITE_BUSY` on the write upgrade)**
...
- [ ] **21. [low] Migration tests run with FK off; round-trip doesn't assert CHECK removal or index recreation**
...
- [ ] **22. [low] Session-expired GETs surface as cryptic JSON parse errors**
