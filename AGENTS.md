...
- All `db.InitDB` connections run through an OTel-instrumented sqlite driver (`github.com/XSAM/otelsql`). Call `db.RegisterOTelDriver(tel.TracerProvider, tel.MeterProvider)` after `telemetry.Setup` and before the first `InitDB` (both binaries do this when telemetry is enabled); without registration `InitDB` uses the raw driver. The test helper (`db.PrepareTestDB`) and the Fiber session store use unwrapped raw drivers.
